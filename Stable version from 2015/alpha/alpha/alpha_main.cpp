...
void HandleTCPClient (int ClientSock)
{
	...
	// Waits the finalization of all threads except threadDetectSpike.	THIS IS TEMPORARY !!!!
	...
	// Waits all threads to finish.			THIS IS THE RIGHT THING TO DO !!!
...
DWORD WINAPI threadDetectSpike(void)
{
	...
	while(1)
	{
		...
		{						
			...
			strcat_s(path,sizeof(path),"C:\\Users\\Username\\Desktop\\Folder\\"); // change de filepath accordingly				
...
{
   //Console::WriteLine( "The Elapsed event was raised at {0}", e->SignalTime ); // DEBUG
   ...
   // DEBUG TO VERIFY THE FIRING RATES AND THE SPEED OF THE WHEELS.
